...
foreach ($service as $key => $value) {
		...
		$check=mysqli_query($conn,"SELECT * FROM temp_medication WHERE visit_id='$visit_id' AND item_id='$value'");
		...
		if (mysqli_num_rows($check)>0) {
			...
			$insert_temp=mysqli_query($conn,"INSERT INTO temp_medication(visit_id,item_id,freq,qty,duration,created_by) VALUES ('$visit_id','$value',1,1,1,'$user_id')");
		}
...
if (mysqli_num_rows($get_bill)>0) {
    ...
    <td>'.$i++.'</td>
    <td>'.$bill['name'].'</td>
    <td><input type="text " value="'.$bill['qty'].'" class="form-control" onkeyup="update_qty('.$bill['id'].',this.value,'.$visit_id.','.$user_id.')" ></td>
    <td><input type="text " value="'.$bill['freq'].'" class="form-control" onkeyup="update_freq'.$bill['id'].',this.value,'.$visit_id.','.$user_id.')" ></td>
    <td><input type="text " value="'.$bill['duration'].'" class="form-control" onkeyup="update_duration'.$bill['id'].',this.value,'.$visit_id.','.$user_id.')" ></td>
